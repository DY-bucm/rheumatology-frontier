...
export function cardHtml(item) {
  ...
  const chineseText = item.abstractZh || "";
  const excerpt = chineseText.length > 360 ? `${chineseText.slice(0, 360)}…` : chineseText;
  return `<article class="research-card">
    <div class="card-meta">
      <span>${escapeHtml(formatDate(item.date))}</span>
      <span>${escapeHtml(item.source || "未知来源")}${item.sourceVerified ? " · 已验证链接" : ""}</span>
      <span class="disease-label">${escapeHtml(item.diseaseZh || "其他")}</span>
    </div>
    <h3><a href="detail.html?id=${encodeURIComponent(item.id)}">${escapeHtml(item.title)}</a></h3>
    ${item.titleZh ? `<p class="title-zh">${escapeHtml(item.titleZh)} <small class="translation-label">DeepSeek 翻译</small></p>` : ""}
    <p class="summary">${escapeHtml(excerpt || (item.abstract ? "中文摘要正在处理中。" : "PubMed 未提供英文摘要。"))}</p>
