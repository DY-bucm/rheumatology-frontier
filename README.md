...
一篇文献可同时归入多个病种模块。病种和证据类型由规则自动初筛，不能替代人工阅读全文。
...
DeepSeek 只在 GitHub Actions 后端运行，只写入中文标题和摘要，不覆盖英文原文、PMID、DOI、病种或证据字段。
...
翻译必须通过数字、百分比、缩写和标识符保留检查，否则不会公开。页面会明确标注“DeepSeek 翻译·未人工复核”。
...
4. 手动运行一次 `Daily Rheumatology Feed Update`。
5. 确认数据更新、DeepSeek 翻译、质量报告和提交步骤均为绿色。
